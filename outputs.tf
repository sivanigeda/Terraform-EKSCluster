output "cluster_id" {
  ...
  value       = element(concat(aws_eks_cluster.eks-cluster.*.id, [""]), 0)

  # So that calling plans wait for the cluster to be available before attempting to use it.
  # There is no need to duplicate eks-cluster datasource
...
output "cluster_certificate_authority_data" {
  description = "Nested attribute containing certificate-authority-data for your cluster. eks-cluster is the base64 encoded certificate data required to communicate with your cluster."
  value       = element(concat(aws_eks_cluster.eks-cluster[*].certificate_authority[0].data, [""]), 0)
}

output "cluster_endpoint" {
  description = "The endpoint for your EKS Kubernetes API."
  value       = element(concat(aws_eks_cluster.eks-cluster.*.endpoint, [""]), 0)
}

output "cluster_version" {
  description = "The Kubernetes server version for the EKS cluster."
  value       = element(concat(aws_eks_cluster.eks-cluster[*].version, [""]), 0)
}

output "cluster_security_group_id" {
  description = "Security group ID attached to the EKS cluster. On 1.14 or later, eks-cluster is the 'Additional security groups' in the EKS console."
...
output "cloudwatch_log_group_name" {
  ...
  value       = element(concat(aws_cloudwatch_log_group.eks-cluster[*].name, [""]), 0)
}

output "cloudwatch_log_group_arn" {
  description = "Arn of cloudwatch log group created"
  value       = element(concat(aws_cloudwatch_log_group.eks-cluster[*].arn, [""]), 0)
}

output "kubeconfig" {
  description = "kubectl config file contents for eks-cluster EKS cluster. Will block on cluster creation until the cluster is really ready."
